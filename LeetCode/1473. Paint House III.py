		...
		if key not in lookup:
			if houses[house] == 0:
				...
				for j in range(len(costs[house])):
					currentPaint = j + 1
					cost = min(cost,
					           costs[house][j] + self.dp(houses, costs, target,
					                                     house + 1,
					                                     currentPaint,
					                                     neighborhood + int(
				...
				lookup[key] = self.dp(houses, costs, target, house + 1,
				                      houses[house], neighborhood + int(
						houses[house] != prevPaint), lookup)
		return lookup[key]
